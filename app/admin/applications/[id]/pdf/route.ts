...
// jsPDF는 동적 import로 로드 (서버 사이드 호환성)
let jsPDF: any;
try {
...
export async function GET(
  ...
  try {
    ...
    if (app.created_at) {
      ...
      doc.text(`신청일시: ${createdDate.toLocaleString('ko-KR')}`, 20, yPosition);
      yPosition += 6;
    }
    yPosition += 5;
    ...
    if (app.schedule_1) {
      ...
      doc.text(`1순위: ${date1} ${time1}`, 20, yPosition);
      yPosition += 6;
    }
    if (app.schedule_2) {
      ...
      doc.text(`2순위: ${date2} ${time2}`, 20, yPosition);
      yPosition += 6;
    }
    yPosition += 5;
