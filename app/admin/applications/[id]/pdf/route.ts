...
<body>
  ...
  ` : ''}
    ...
    // 외부 HTML to PDF API 사용 (무료 옵션: htmlpdfapi.com 또는 api2pdf.com)
    // 또는 클라이언트에서 처리하도록 HTML 반환
    
    // 옵션 1: HTML을 반환하고 클라이언트에서 PDF 생성 (권장)
    // 클라이언트에서 jsPDF의 html() 메서드 사용하면 한글 폰트가 제대로 적용됨
